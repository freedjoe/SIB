...
CREATE TABLE IF NOT EXISTS fiscal_years (
    ...
    status TEXT CHECK (
        status IN ('planning', 'active', 'closed', 'draft')
    ) DEFAULT 'draft',
    description TEXT
);
-- 2. Ministries
...
CREATE TABLE IF NOT EXISTS allocations (
    ...
    reference_type TEXT CHECK (
        reference_type IN ('portfolio', 'program', 'action', 'operation')
    ),
    ...
    source TEXT CHECK (
        source IN (
            ...
            'cas_302_145',
            -- Compte d’affectation spéciale 302-145
            'external_funding',
            -- Financement extérieur (emprunts, dons, etc.)
            'donation',
            -- Dons spécifiques
            'interministeriel_transfer',
            -- Virements interportefeuilles
            'reallocation',
...
CREATE TABLE IF NOT EXISTS budget_modifications (
    ...
    action_id UUID REFERENCES actions(id),
    type TEXT CHECK (
        type IN ('new_entry', 'revaluation', 'reallocation')
    ),
    reason TEXT,
    amount NUMERIC,
    decision TEXT
);
-- 11. Engagements (marché, contrat, décision)
...
FROM (
        VALUES -- fiscal_years
            ...
            (
                'fiscal_years',
                'status',
                'TEXT CHECK (status IN (''planning'', ''active'', ''closed'', ''draft'')) DEFAULT ''draft'''
            ),
            ...
            (
                'portfolios',
                'status',
                'TEXT CHECK (status IN (''draft'', ''active'', ''archived'')) DEFAULT ''draft'''
            ),
            ...
            (
                'programs',
                'portfolio_id',
                'UUID REFERENCES portfolios(id) ON DELETE CASCADE'
            ),
            ...
            (
                'programs',
                'type',
                'VARCHAR(20) CHECK (type IN (''program'', ''subprogram''))'
            ),
            (
                'programs',
                'parent_id',
                'UUID REFERENCES programs(id)'
            ),
            ...
            (
                'subprograms',
                'program_id',
                'UUID REFERENCES programs(id) ON DELETE CASCADE'
            ),
            ...
            (
                'actions',
                'subprogram_id',
                'UUID REFERENCES subprograms(id)'
            ),
            ...
            (
                'actions',
                'type',
                'TEXT CHECK (type IN (''Centralized'', ''Decentralized'', ''Unique'', ''Programmed'', ''Complementary''))'
            ),
            ...
            (
                'actions',
                'status',
                'TEXT CHECK (status IN (''proposed'', ''validated'', ''draft'', ''active'', ''archived'')) DEFAULT ''draft'''
            ),
            ...
            (
                'operations',
                'portfolio_id',
                'UUID REFERENCES actions(id) ON DELETE CASCADE'
            ),
            (
                'operations',
                'program_id',
            ...
            (
                'operations',
                'budget_title_id',
                'UUID REFERENCES budget_titles(id)'
            ),
            ...
            (
                'operations',
                'execution_mode',
                'TEXT CHECK (execution_mode IN (''state'', ''delegation'', ''PPP''))'
            ),
            (
                'operations',
                'project_status',
                'TEXT CHECK (project_status IN (''not_started'', ''planned'', ''in_progress'', ''completed'', ''on_hold'', ''suspended'', ''delayed'', ''canceled'', ''completely_frozen'', ''partially_frozen''))'
            ),
            (
                'operations',
                'status',
                'TEXT CHECK (status IN (''draft'', ''submitted'', ''reviewed'', ''approved'', ''rejected'')) DEFAULT ''draft'''
            ),
            -- operation_cps
            (
                'operation_cps',
                'operation_id',
                'UUID REFERENCES operations(id) ON DELETE CASCADE'
            ),
            (
                'operation_cps',
                'fiscal_year_id',
                'UUID REFERENCES fiscal_years(id)'
            ),
            (
                'operation_cps',
                'month',
            ...
            (
                'operation_cps',
                'montant_cp',
                'NUMERIC NOT NULL CHECK (montant_cp >= 0)'
            ),
            ...
            (
                'allocations',
                'reference_type',
                'TEXT CHECK (reference_type IN (''portfolio'', ''program'', ''action'', ''operation''))'
            ),
            ...
            (
                'allocations',
                'state',
                'TEXT CHECK (state IN (''initial'', ''revised''))'
            ),
            (
                'allocations',
                'source',
                'TEXT CHECK (source IN (''budget_general'', ''cas_302_145'', ''external_funding'', ''donation'', ''interministeriel_transfer'', ''reallocation'', ''revaluation''))'
            ),
            ...
            (
                'budget_modifications',
                'type',
                'TEXT CHECK (type IN (''new_entry'', ''revaluation'', ''reallocation''))'
            ),
            ...
            (
                'engagements',
                'status',
                'TEXT CHECK (status IN (''proposed'', ''validated'', ''liquidated'', ''draft'', ''submitted'', ''reviewed'', ''approved'', ''rejected'')) DEFAULT ''draft'''
            ),
            ...
            (
                'revaluations',
                'engagement_id',
                'UUID REFERENCES engagements(id)'
            ),
            ...
            (
                'payments',
                'status',
                'TEXT CHECK (status IN (''draft'', ''pending'', ''approved'', ''paid'', ''rejected'')) DEFAULT ''draft'''
            ),
            (
                'payments',
                'operation_id',
            ...
            (
                'cp_consumptions',
                'mobilisation_id',
                'UUID REFERENCES cp_mobilisations(id)'
            ),
            ...
            (
                'payment_requests',
                'frequency',
                'TEXT CHECK (frequency IN (''monthly'', ''quarterly'', ''annual'')) DEFAULT ''annual'''
            ),
            ('payment_requests', 'justification', 'TEXT'),
            (
                'payment_requests',
                'status',
                'TEXT CHECK (status IN (''draft'', ''pending'', ''reviewed'', ''approved'', ''rejected'')) DEFAULT ''draft'''
            ),
            ...
            (
                'requests',
                'ministry_id',
                'UUID REFERENCES ministries(id) ON DELETE CASCADE'
            ),
            (
                'requests',
                'fiscal_year_id',
            ...
            (
                'requests',
                'type',
                'TEXT CHECK (type IN (''New registration'', ''Revaluation'', ''Payment credit'', ''Allocation'', ''Reallocation'', ''Transfer'', ''Additional request'', ''Previous commitments'', ''Balance to regularize'', ''Other''))'
            ),
            (
                'requests',
                'priority',
            ...
            (
                'documents',
                'access_level',
                'TEXT CHECK (access_level IN (''public'', ''private'', ''restricted'')) DEFAULT ''private'''
            ),
            -- Settings
            ...
            (
                'settings',
                'value_type',
                'TEXT CHECK (value_type IN (''string'', ''number'', ''boolean'', ''json'')) DEFAULT ''string'''
            ),
            ...
            (
                'tag_associations',
                'tag_id',
                'UUID REFERENCES tags(id) ON DELETE CASCADE'
            ),
            ...
            (
                'milestones',
                'status',
                'TEXT CHECK (status IN (''pending'', ''in_progress'', ''completed'', ''delayed'', ''cancelled'')) DEFAULT ''pending'''
            ),
            (
                'milestones',
                'progress',
                'NUMERIC CHECK (progress >= 0 AND progress <= 100) DEFAULT 0'
            ),
            (
                'milestones',
                'created_at',
    ...
    ) AS columns(table_name, column_name, column_type) LOOP IF NOT EXISTS (
        SELECT 1
